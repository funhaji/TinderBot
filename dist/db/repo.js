...
export async function setLanguage(userId, lang) {
    await query(`UPDATE users SET language = $2 WHERE id = $1`, [userId, lang]);
}
export async function getUserByTelegramId(telegramId) {
    const res = await query(`SELECT id, telegram_id, username, language, is_banned,
            diamond_balance, face_verification_status, referral_bonus_paid, referred_by,
            referral_code, badge_verified, badge_vip
...
export async function banUser(userId, reason) {
    await query(`UPDATE users SET is_banned = true, banned_at = now(), banned_reason = $2 WHERE id = $1`, [userId, reason.slice(0, 500)]);
}
export async function unbanUser(userId) {
    await query(`UPDATE users SET is_banned = false, banned_at = NULL, banned_reason = NULL WHERE id = $1`, [userId]);
}
export async function countUsers() {
    const res = await query(`SELECT COUNT(*)::text AS c FROM users`);
    return Number(res.rows[0]?.c ?? 0);
}
export async function countMatches() {
    const res = await query(`SELECT COUNT(*)::text AS c FROM matches WHERE unmatched_at IS NULL`);
    return Number(res.rows[0]?.c ?? 0);
}
export async function countOpenReports() {
    const res = await query(`SELECT COUNT(*)::text AS c FROM reports WHERE status = 'open'`);
    return Number(res.rows[0]?.c ?? 0);
}
export async function listTelegramIdsForBroadcast() {
    const res = await query(`SELECT telegram_id::text FROM users WHERE is_banned = false ORDER BY id`);
...
export async function findMysteryWaitUser(params) {
    ...
    `, [params.excludeUserId]);
        ...
        for (const row of res.rows) {
            ...
            const theirPrefs = (row.preferences ?? {});
            ...
            if (!mysterySoughtGenderMatches(params.soughtGender, row.gender))
                continue;
            if (!mysterySoughtGenderMatches(theirSoughtGender, params.myGender))
                continue;
            const ageDiff = Math.abs(params.myAge - Number(row.age));
            if (params.ageRangeClose && ageDiff > 5)
                continue;
            if (theirAgeRangeClose && ageDiff > 5)
                continue;
            const theirCountry = row.country ?? null;
            if (params.wantSameCountry && (myCountryNorm === null || myCountryNorm !== theirCountry))
                continue;
            if (theirWantSameCountry && (theirCountry === null || myCountryNorm !== theirCountry))
                continue;
            const upd = await q(`UPDATE sessions SET state='idle', payload='{}'::jsonb, updated_at=now()
         WHERE user_id=$1 AND state='mystery_wait'`, [row.user_id]);
            if (upd.rowCount === 1)
...
export async function expireMysteryWaitSessions() {
...
export async function expireMysteryVoteSessions() {
    const res = await query(`SELECT s.user_id, u.telegram_id::text, COALESCE(u.language, 'fa') AS language,
...
export async function ensureSessionRow(userId) {
    await query(`
...
export async function getSession(userId) {
    const res = await query(`SELECT state, payload FROM sessions WHERE user_id = $1`, [userId]);
    if (!res.rows[0])
        return { state: "idle" };
    const row = res.rows[0];
    return { state: row.state, payload: row.payload };
}
export async function setSession(userId, s) {
...
export async function resetSession(userId) {
    await query(`UPDATE sessions SET state='idle', payload='{}'::jsonb, updated_at=now() WHERE user_id=$1`, [userId]);
}
...
export async function mergeProfilePreferences(userId, patch) {
...
export async function replacePhotos(userId, fileIds) {
    await tx(async (q) => {
...
export async function getPrimaryPhoto(userId) {
    const res = await query(`SELECT file_id FROM photos WHERE user_id=$1 ORDER BY is_primary DESC, id ASC LIMIT 1`, [userId]);
    return res.rows[0]?.file_id ?? null;
}
export async function listPhotoFileIds(userId) {
    const res = await query(`SELECT file_id FROM photos WHERE user_id=$1 ORDER BY is_primary DESC, id ASC`, [userId]);
    return res.rows.map((r) => r.file_id);
}
export async function setUserInterests(userId, keys) {
    await tx(async (q) => {
        await q(`DELETE FROM user_interests WHERE user_id=$1`, [userId]);
        if (keys.length === 0)
            return;
        const res = await q(`SELECT id, key FROM interests WHERE key = ANY($1::text[])`, [keys]);
...
export async function listInterests() {
    const res = await query(`SELECT key, fa_label, en_label FROM interests ORDER BY key`);
    return res.rows;
}
export async function getUserInterestKeys(userId) {
    const res = await query(`
    SELECT i.key
...
export async function swipe(params) {
    await query(`INSERT INTO swipes (swiper_id, target_id, direction) VALUES ($1,$2,$3)
     ON CONFLICT (swiper_id, target_id) DO UPDATE SET direction=EXCLUDED.direction, created_at=now()`, [params.swiperId, params.targetId, params.direction]);
}
export async function hasLiked(swiperId, targetId) {
    const res = await query(`SELECT direction FROM swipes WHERE swiper_id=$1 AND target_id=$2`, [swiperId, targetId]);
    return res.rows[0]?.direction === 1;
}
export async function ensureMatch(userId1, userId2) {
...
export async function getTelegramIdByUserId(userId) {
    const res = await query(`SELECT telegram_id::text FROM users WHERE id=$1`, [userId]);
    return res.rows[0] ? Number(res.rows[0].telegram_id) : null;
}
export async function createReport(params) {
    await query(`INSERT INTO reports (reporter_id, target_id, reason, status) VALUES ($1,$2,$3,'open')
     ON CONFLICT (reporter_id, target_id) DO UPDATE SET
       reason = EXCLUDED.reason,
       status = 'open',
       resolved_at = NULL,
       resolved_by_telegram = NULL,
       created_at = now()`, [params.reporterId, params.targetId, params.reason]);
}
export async function listOpenReports(limit, offset) {
    const res = await query(`
...
export async function blockUser(blockerId, blockedId) {
    await query(`INSERT INTO user_blocks (blocker_id, blocked_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [blockerId, blockedId]);
}
...
export async function userStats(userId) {
    const likesSent = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE swiper_id = $1 AND direction = 1`, [userId]);
    const likesReceived = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE target_id = $1 AND direction = 1`, [userId]);
    const matches = await query(`SELECT COUNT(*)::text AS c FROM matches WHERE unmatched_at IS NULL AND (user_a = $1 OR user_b = $1)`, [userId]);
    return {
...
export async function hasSwiped(swiperId, targetId) {
    const res = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE swiper_id = $1 AND target_id = $2`, [swiperId, targetId]);
    return Number(res.rows[0]?.c ?? 0) > 0;
}
export async function hasMatchBetween(userId, otherId) {
    const a = Math.min(userId, otherId);
    const b = Math.max(userId, otherId);
    const res = await query(`SELECT COUNT(*)::text AS c FROM matches WHERE unmatched_at IS NULL AND user_a = $1 AND user_b = $2`, [a, b]);
    return Number(res.rows[0]?.c ?? 0) > 0;
}
export async function getSystemSettingBool(key, defaultVal) {
...
export async function getSystemSettingNumber(key, defaultVal) {
    const res = await query(`SELECT value_json FROM system_settings WHERE key = $1`, [key]);
    const v = res.rows[0]?.value_json;
...
export async function insertMessageLog(params) {
    ...
    if (!enabled)
        return;
    await query(`INSERT INTO message_logs (direction, user_id, telegram_user_id, chat_id, message_id, update_type, text_preview, payload)
...
export async function insertProfileImpression(viewerId, shownUserId) {
    await query(`INSERT INTO profile_impressions (viewer_id, shown_user_id) VALUES ($1, $2)`, [
        viewerId,
        shownUserId,
    ]);
}
export async function addPermanentHide(hiderId, hiddenId) {
    await query(`INSERT INTO user_permanent_hides (hider_id, hidden_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [hiderId, hiddenId]);
}
export async function resetUserNopes(userId) {
    await query(`DELETE FROM swipes WHERE swiper_id = $1 AND direction = 2`, [userId]);
}
export async function getPendingFaceSubmissionUserId(submissionId) {
    const res = await query(`SELECT user_id::text FROM face_verification_submissions WHERE id = $1 AND status = 'pending'`, [submissionId]);
    return res.rows[0] ? Number(res.rows[0].user_id) : null;
}
export async function createFaceSubmission(userId, photoFileId) {
    await query(`UPDATE users SET face_verification_status = 'pending' WHERE id = $1 AND face_verification_status <> 'approved'`, [userId]);
    const res = await query(`INSERT INTO face_verification_submissions (user_id, photo_file_id, status) VALUES ($1, $2, 'pending') RETURNING id::text`, [userId, photoFileId]);
    return Number(res.rows[0].id);
}
export async function listPendingFaceSubmissions(limit) {
    const res = await query(`
...
export async function approveFaceSubmission(params) {
    await tx(async (q) => {
        const sub = await q(`SELECT user_id::text FROM face_verification_submissions WHERE id = $1 AND status = 'pending'`, [params.submissionId]);
...
export async function rejectFaceSubmission(params) {
    await tx(async (q) => {
        ...
        if (!uid)
            return;
        await q(`UPDATE face_verification_submissions SET status = 'rejected', reviewed_at = now(),
       reviewer_telegram_id = $2, reject_reason = $3 WHERE id = $1`, [params.submissionId, params.reviewerTelegramId, params.reason.slice(0, 200)]);
        await q(`UPDATE users SET face_verification_status = 'rejected' WHERE id = $1 AND face_verification_status <> 'approved'`, [uid]);
    });
}
export async function adjustDiamondBalance(params) {
    await tx(async (q) => {
        await q(`INSERT INTO diamond_ledger (user_id, delta, reason, ref_json, admin_telegram_id)
...
export async function markReferralBonusPaid(userId) {
    await query(`UPDATE users SET referral_bonus_paid = true WHERE id = $1`, [userId]);
}
export async function extendedUserStats(userId) {
    const io = await query(`SELECT COUNT(*)::text AS c FROM profile_impressions WHERE viewer_id = $1`, [userId]);
    const ii = await query(`SELECT COUNT(*)::text AS c FROM profile_impressions WHERE shown_user_id = $1`, [userId]);
    const ls = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE swiper_id = $1 AND direction = 1`, [userId]);
    const lr = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE target_id = $1 AND direction = 1`, [userId]);
    const ds = await query(`SELECT COUNT(*)::text AS c FROM swipes WHERE swiper_id = $1 AND direction = 2`, [userId]);
    const m = await query(`SELECT COUNT(*)::text AS c FROM matches WHERE unmatched_at IS NULL AND (user_a = $1 OR user_b = $1)`, [userId]);
    const d = await query(`SELECT diamond_balance::text FROM users WHERE id = $1`, [userId]);
    ...
    const attractivenessPct = impressionsIn > 0 ? Math.round((10000 * likesReceived) / impressionsIn) / 100 : 0;
    return {
...
export async function listDiamondLedgerTail(userId, limit) {
    const res = await query(`SELECT delta::text, reason, created_at::text FROM diamond_ledger WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2`, [userId, limit]);
    return res.rows;
}
export async function getUserByReferralCode(codeRaw) {
    ...
    const res = await query(`SELECT id, telegram_id, username, language, is_banned,
            diamond_balance, face_verification_status, referral_bonus_paid, referred_by,
            referral_code, badge_verified, badge_vip
     FROM users WHERE lower(referral_code) = lower($1) LIMIT 1`, [code]);
    return res.rows[0] ?? null;
}
export async function socialPairAllowed(aId, bId) {
    const [a, b] = await Promise.all([getProfile(aId), getProfile(bId)]);
    if (!a || !b)
        return false;
    if (!orientationMutualOk(a.preferences?.orientation ?? null, a.gender, b.preferences?.orientation ?? null, b.gender))
        return false;
    if (!seekGenderMatchesProfile(a.preferences?.seek_genders ?? null, b.gender))
        return false;
    if (!seekGenderMatchesProfile(b.preferences?.seek_genders ?? null, a.gender))
        return false;
    return true;
}
export async function countReferralsWithProfile(referrerUserId) {
    const res = await query(`
...
export async function applyReferralMilestonesForReferrer(referrerUserId) {
    ...
    await query(`UPDATE users SET badge_vip = $2, badge_verified = $3 WHERE id = $1`, [referrerUserId, vip, verified]);
    return { vip, verified };
}
export async function listReferralFileRewards() {
    const res = await query(`SELECT id, sort_order, min_referrals, caption_fa, caption_en, file_id, created_at::text
...
export async function insertReferralFileReward(params) {
    const res = await query(`INSERT INTO referral_file_rewards (sort_order, min_referrals, caption_fa, caption_en, file_id)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id::text`, [params.sortOrder ?? 0, params.minReferrals, params.captionFa, params.captionEn, params.fileId]);
...
export async function claimReferralFileReward(userId, rewardId) {
    const n = await countReferralsWithProfile(userId);
    const ok = await query(`SELECT id::text FROM referral_file_rewards WHERE id=$1 AND min_referrals <= $2`, [rewardId, n]);
    if (!ok.rows[0])
        return false;
...
export async function getAdminDashboardStats() {
    const u = await query(`SELECT COUNT(*)::text AS c FROM users`);
    const a = await query(`SELECT COUNT(*)::text AS c FROM users WHERE last_seen_at > now() - interval '24 hours'`);
    const m = await query(`SELECT COUNT(*)::text AS c FROM matches WHERE created_at > now() - interval '24 hours'`);
    const mw = await query(`SELECT COUNT(*)::text AS c FROM sessions WHERE state = 'mystery_wait'`);
    const mv = await query(`SELECT COUNT(*)::text AS c FROM sessions WHERE state = 'mystery_vote'`);
    return {
...
export async function adminGenderDistribution() {
    const res = await query(`SELECT gender::text AS key, COUNT(*)::text AS c FROM profiles GROUP BY gender ORDER BY COUNT(*) DESC`);
    return res.rows.map((r) => ({ key: r.key ?? "null", c: Number(r.c ?? 0) }));
}
export async function adminOrientationDistribution() {
    const res = await query(`SELECT COALESCE(preferences->>'orientation','null') AS key, COUNT(*)::text AS c
     FROM profiles
     GROUP BY 1
     ORDER BY COUNT(*) DESC`);
    return res.rows.map((r) => ({ key: r.key ?? "null", c: Number(r.c ?? 0) }));
