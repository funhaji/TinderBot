...
export function physSexFromGender(gender) {
    if (!gender)
        return null;
    switch (gender) {
        case "m":
...
export function effectiveOrientation(orientation, gender) {
    const o = orientation?.trim() || "";
    if (!o || o === "skip")
        return "any";
    if (o === "bi" || o === "bisexual" || o === "other")
        return "any";
...
export function orientationAcceptsViewer(viewerOrientationEff, viewerPhys, targetPhys) {
    if (viewerOrientationEff === "any")
        return true;
    if (viewerOrientationEff === "straight") {
        if (viewerPhys === "m" && targetPhys === "f")
            return true;
        if (viewerPhys === "f" && targetPhys === "m")
            return true;
        if (viewerPhys == null || targetPhys == null)
            return true;
        return false;
    }
    if (viewerOrientationEff === "gay") {
        if (viewerPhys === "m" && targetPhys === "m")
            return true;
        if (viewerPhys == null || targetPhys == null)
            return true;
        return false;
...
export function orientationMutualOk(aOrientation, aGender, bOrientation, bGender) {
...
export function seekGenderMatchesProfile(seek, targetGender) {
    const set = seek?.filter(Boolean) ?? [];
    if (set.length === 0)
        return true;
    const tp = physSexFromGender(targetGender);
    if (tp === "m" && set.includes("m"))
        return true;
    if (tp === "f" && set.includes("f"))
        return true;
    if (tp === null && set.includes("x"))
        return true;
    return false;
}
export function mysterySoughtGenderMatches(sought, targetGender) {
    if (!sought || sought === "any")
        return true;
...
export function ageWindowOverlaps(aAge, aMin, aMax, bAge, bMin, bMax) {
...
export function orientationLabel(lang, o) {
    if (!o || o === "skip")
        return "—";
    const map = {
