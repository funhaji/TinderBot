...
function adminValueLabel(lang, value) {
    ...
    if (key === "no")
        return lang === "fa" ? "خیر" : "No";
    if (key === "null" || key === "none" || key === "")
        return "—";
    return value;
}
function adminGenderLabel(lang, value) {
    const map = {
        boy: { fa: "پسر", en: "Boy" },
...
function adminOrientationLabel(lang, value) {
    const map = {
...
function formatDistribution(lang, rows, labeler) {
    if (rows.length === 0)
        return "—";
