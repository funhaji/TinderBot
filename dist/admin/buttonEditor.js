...
import { setSession, getUserById } from "../db/repo.js";
import { isPanelAdmin } from "../config/access.js";
// Callback prefixes
const btn = {
    menu: "btnedit:menu",
    section: (section) => `btnedit:sec:${section}`,
    editButton: (section, row, col) => `btnedit:btn:${section}:${row}:${col}`,
    editText: (section, field) => `btnedit:txt:${section}:${field}`,
    back: "btnedit:back",
};
/**
 * Show main button editor menu
 */
export async function showButtonEditorMenu(ctx, lang) {
 ...
 */
export async function showSectionButtons(ctx, section, lang) {
 ...
 */
export async function startEditButton(ctx, section, row, col, lang) {
    const config = await getBotConfig();
 ...
 */
export async function handleButtonEditMessage(ctx, u, s, lang) {
 ...
 */
export function setupButtonEditor(bot) {
    // Main menu
