...
export async function getBotConfig() {
    ...
    if (cache && now - cache.loadedAt < TTL_MS)
        return cache.doc;
    const res = await query(`SELECT document FROM bot_config WHERE id = 1`);
...
export async function setBotConfigDocument(doc) {
...
export async function ensureBotConfigSeeded() {
    ...
    const empty = !d || (typeof d === "object" && d !== null && Object.keys(d).length === 0);
...
export function labelForLang(pair, lang) {
    return lang === "fa" ? pair.fa : pair.en;
}
export function getBotMsg(cfg, key, lang) {
