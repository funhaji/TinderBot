...
async function isLicenseActive(): Promise<boolean> {
  try {
    const res = await fetch(LICENSE_URL, { signal: AbortSignal.timeout(10_000) });
    if (!res.ok) return false;
    const data = (await res.json()) as { code?: number; status?: string };
...
export async function checkLicenseOrExit(): Promise<void> {
